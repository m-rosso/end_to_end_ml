...
class TreatMissings:
    ...
    def fit_transform(self, data: pd.DataFrame, training_data: pd.DataFrame = None) -> pd.DataFrame:
        """
        ...
        if self.vars_to_treat is None:
            self.vars_to_treat = [c for c in list(data.columns) if (c not in self.drop_vars) & (c not in self.cat_vars) &
                                  (data[c].isnull().sum() > 0)]
        ...
        if self.method=='create_binary':
            ...
            stats = self.calculate_stat(training_data=training_data, vars_to_treat=self.vars_to_treat, drop_vars=self.drop_vars,
                                        cat_vars=self.cat_vars, statistic=self.statistic)
            transf_data = self.impute_stat(data=transf_data, stats=stats)
    ...
    def transform(self, data_list: Union[List[pd.DataFrame], Tuple[pd.DataFrame]], training_data: pd.DataFrame) -> pd.DataFrame:
        ...
        for op in self.operations:
            if 'fit' not in dir(op):
                if 'training_data' in op.fit_transform.__code__.co_varnames:
                    transf_training_data = op.fit_transform(data=transf_training_data, training_data=transf_training_data)

                    # Loop over application data:
                    for i in range(len(transf_data_list)):
                        transf_data_list[i] = op.fit_transform(data=transf_data_list[i], training_data=transf_training_data)
